...
namespace Client1712
{
    ...
    public partial class SignUp : Form
    {
        ...
        //Receives info and message the client accordinally
        {
        ...
        private void btSignUp_Click(object sender, EventArgs e) 
        {
            ...
            {
                if (LbConfirmPass_E.Visible || LBPassword_E.Visible)
                    MessageBox.Show("Error : passwords are not matching");
                if (LBUsername_E.Visible)
                    MessageBox.Show("Error : username contains digits");
                if (LBFirstname_E.Visible)
                    MessageBox.Show("Error : first name contains digits");
                if (LBLastName_E.Visible)
                    MessageBox.Show("Error : last name contains digits");
                if (LBEmail_E.Visible)
                    MessageBox.Show("Error : email formatted incorrectly");
            }
            else
            {
                SendInfrmaionToServer("Register;" + TBFirstName.Text + ";" + TBName.Text + ";" + TBEmail.Text
